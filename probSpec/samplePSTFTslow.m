function [Y,X] = samplePSTFTslow(Lam,Var,Om,vary,T);
  
  % [Y,X] = samplePSTFTslow(Lam,Var,Om,vary,T);
  %
  ...
  %
  % See samplePSTFT.m for a fast version which uses the spectrum
  % for efficiency.
  %
  % Inputs
  ...
  for d=1:D
    ...
    x1 = sampleARTau(Lam(d),Var(d),T,1);
    x2 = sampleARTau(Lam(d),Var(d),T,1);
