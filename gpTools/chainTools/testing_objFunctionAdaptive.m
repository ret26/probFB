function [f,df] = testing_objFunctionAdaptive(theta,K,covfunc,X,Y,tau1,tau2,missingInd,weights)

% [fest,~] = testing_predictAdaptive(theta,K,covfunc,X,Y,tau1,tau2,missingInd);
...
smoothing = 1;
verbose = 0;
[f,df] = testing_kalmanAdaptive(At,Ct,Qt,Rt,x0,P0,yKalman,verbose,...
    1-smoothing,weights,dAt,dCt,dQt,dRt,dP0);
f = -f/(noblks*tau1);
df = -df/(noblks*tau1);
end
