function varargout = ...
    testing_kalmanAdaptive(A,Ct,Q,Rt,x0,P0,Yt,varargin)
...
for t=1:T
    ...
    if length(varargin) >= 3
        ...
        if e2
            ...
            lik=lik + weights(t)*(-Dt/2*log(2*pi)-logdetP-0.5*sum(sum(Ydiff.*(invP*Ydiff))));
        end
    ...
    for i = 1:T
        ...
        for j = 1:noVar
            if i==1
                ...
                dlik(j) = dlik(j) + weights(i)*(M15 - 1/2*(sum(sum(M16.*sig11')) + mu1'*M16*mu1));
            else
                ...
                Lb3 = Lb31 + Lb32;
                dlik(j) = dlik(j) + weights(i)*(M5{j} + Lb2 + Lb3 + Lb4);
            end
            ...
            if j==noVar % dRt diag and dCt = 0
                ...
                dlik(j) = dlik(j) + weights(i)*(Z1 + Z2 + Z3 + Z4 + Z5);
            else
                if j<=D
                    k = ceil(j/2);
                else
                ...
                Z2 = 1/2*RinvY'*dRt{i,j}*RinvY;
                dCmu1 = dCt{i,j}*mu1((k-1)*tau2 + (1:tau2));
                Z3 = -RinvY'*dRt{i,j}*RinvCmu1 + RinvY'*dCmu1;
                
                Bk = RinvCsig11(:,(k-1)*tau2 + (1:tau2));
                dCktrBk = dCt{i,j}'*Bk;
...
if length(varargin) >= 3
    ...
    varargout{1} = Xfint;
    varargout{2} = Pfint;
    if exist('A4','var'); varargout{3} = A4; end
end