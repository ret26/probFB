function test_suite = test_kalman_GTFtNMF_FB_hier_EM
  initTestSuite;

% Tests: 
%
% [lik,VV,RVV] = kalman_GTFtNMF_FB_hier_EM(y,Amp,lamx,varx,omx,vary);
%
...
RVV1(1,1) = R1(1,1)*(sigmaPost(1,5)+muPost(1)*muPost(5)) ...
            + R1(1,2)*(sigmaPost(2,5)+muPost(2)*muPost(5)) ...
            + R1(2,1)*(sigmaPost(1,6)+muPost(1)*muPost(6)) ...
            + R1(2,2)*(sigmaPost(2,6)+muPost(2)*muPost(6));

RVV1(2,1) = R2(1,1)*(sigmaPost(3,7)+muPost(3)*muPost(7)) ...
            + R2(1,2)*(sigmaPost(4,7)+muPost(4)*muPost(7)) ...
            + R2(2,1)*(sigmaPost(3,8)+muPost(3)*muPost(8)) ...
            + R2(2,2)*(sigmaPost(4,8)+muPost(4)*muPost(8));

RVV1(1,2) = R1(1,1)*(sigmaPost(5,9)+muPost(5)*muPost(9)) ...
            + R1(1,2)*(sigmaPost(6,9)+muPost(6)*muPost(9)) ...
            + R1(2,1)*(sigmaPost(5,10)+muPost(5)*muPost(10)) ...
            + R1(2,2)*(sigmaPost(6,10)+muPost(6)*muPost(10));

RVV1(2,2) = R2(1,1)*(sigmaPost(7,11)+muPost(7)*muPost(11)) ...
            + R2(1,2)*(sigmaPost(8,11)+muPost(8)*muPost(11)) ...
            + R2(2,1)*(sigmaPost(7,12)+muPost(7)*muPost(12)) ...
            + R2(2,2)*(sigmaPost(8,12)+muPost(8)*muPost(12));


[lik2,VV2,RVV2] = kalman_GTFtNMF_FB_hier_EM(Y(:),Amp,lamx,varx,om,vary);
...
assertVectorsAlmostEqual(RVV1,RVV2,'absolute',tol,0)
