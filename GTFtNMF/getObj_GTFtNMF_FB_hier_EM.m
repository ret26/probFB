function [obj,dobj] = getObj_GTFtNMF_FB_hier_EM(zlogw,VV,RVV,lamv,varv,lenx,mux,varx,preZ,tol);

% function [obj,dobj] = getObj_GTFtNMF_FB_hier_EM(zlogw,VV,RVV,lamv,varv,lenx,mux,varx,tol);
% 
% EM learning of GTF-tNMF with hierarchical filter bank formulation
% for optimising both over the envelopes and the weights. I.e. for
% performing inference and learning.
%
...
%
% INPUTS
% zlogW = vectorised matrices of temporal basis functions and W [T*K+D*K,1]
...
% split it up into smaller modules.
  ...
  obj = (obj1+obj2)/T;
  dobj = ([dobj1dZ(:)+dobj2dZ(:);dobj1dW(:)])/T;
