function test_suite = test_mPAD_infer_X1X2_basis_func_HMCMC
  initTestSuite;

% Tests: 
%
% [X1,X2,Info] = mPAD_infer_X1X2_basis_func(y,Z2,Params,varargin)
%
...
X2Init = -3*ones(T,K);
[X2Samp,acc] = mPAD_infer_X1X2_basis_func_HMCMC(y,X2Init,Params,opts);
...
% Modulator Processes
Len2 = [20,50]; % time constant in samples of modulators
Mu2 = -ones(D,1);
Var2 = 9*ones(K,1);
...
opts.numIts = 40; 
Z2InitMAP = ones(size(X2))*0;

[X1Est1,X2Est1,Info] = mPAD_infer_X1X2_basis_func(y,Z2InitMAP,Params,opts);

optsHMCMC.no_samps = 50; 
optsHMCMC.no_leap = 5; 
optsHMCMC.epsilon = 0.05;

Z2Init = ones(size(X2))*0;
Z2Init = Info.Z2;
X2Init = Z2_to_X2(Z2Init,Params.Len2,Params.Var2,7);

[X2Est,acc] = mPAD_infer_X1X2_basis_func_HMCMC(y,Z2Init,Params,optsHMCMC);

AEst = log(1+exp(X2Est(1:T,:)*G'+ones(T,1)*Mu2'));
AInit = log(1+exp(X2Init(1:T,:)*G'+ones(T,1)*Mu2'));
