...
% function [Obj,dObj] = getObj_mPAD_noG(x2,y,fftCov,Params);
%
% Gets the mPAD objective function and derivatives wrt X2 (not
% G). This function is used for estimating X2 when we know G.
%
% INPUTS
% x2 = Transformed envelopes [Tx*K,1] (gets reshaped
%      into [Tx,K]) 
% y = Obsevations [T,1] 
% fftCov = FFTs of the covariance matrices, [Tx,K]
% Params = structure containing the following parameters
%   G = modulation features [D,K]
...
% OUTPUTS
% Obj = objective
% dObjdX2 = derivative of the objective by X2, [2(T-1)*K,1] 
%
% This function is too long - in the future it would be better to
% split it up into smaller modules. Lots of code is repeated here
% from getObj_mPAD_fixG.m Using modules should allow us to have
% just one version of the code and not too copies
